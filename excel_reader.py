...
def build_or_update_corpus(new_df, progress_container):
    ...
    try:
        ...
        if not new_df.empty:
            ...
            # Unisce il nuovo DataFrame a quello esistente
            corpus_df = pd.concat([corpus_df, new_df], ignore_index=True)

            # Rimuove i duplicati per mantenere il corpus pulito
            corpus_df.drop_duplicates(inplace=True)
            progress_container(f"Corpus aggiornato. Righe totali dopo l'aggiornamento: {len(corpus_df)}", "success")

            # Salva il corpus aggiornato in formato Parquet
            corpus_df.to_parquet(CORPUS_FILE, index=False)
            progress_container("Corpus salvato su disco.", "success")
        else:
